...
int main(int argc, char** argv) {
    ...
    unsigned char           buff[sizeof(struct ip) + sizeof(struct tcphdr)
                                     + sizeof(struct phdr)] = {0};

    /* Wskaznik na naglowek IP (w buforze okreslonym przez 'buff'): */
    struct ip               *ip_header      = (struct ip *)buff;

    /* Wskaznik na naglowek TCP (w buforze okreslonym przez 'buff'): */
    struct tcphdr           *tcp_header     = (struct tcphdr *)
            (buff + sizeof(struct ip));
    /* Wskaznik na pseudo-naglowek (w buforze okreslonym przez 'buff'): */
    ...
    // Ustawienie data offset (ile zajmuje header x4 bajtow)
    tcp_header->doff = 5;

    // Wyzerowanie niuzywanej przestrzeni
    tcp_header->res1 = 0;

    // Ustawianie numeru syn (chyba nie ma znaczenia jaki)
    tcp_header->th_seq = 0;

    // Wyzerowanie numeru ack
    tcp_header->th_ack = 0;

    // ustawienie flagi SYN
    tcp_header->th_flags = 0x02;

    // maksymalny rozmiar danych wyslanych przez klientach, 5840 w pelni wystarcza (typowa wartosc)
    tcp_header->th_win = htons(5840);

    // Wskaznik na urgent data (niuzywamy w tym zadaniu)
    ...
    /* Rozmiar naglowka TCP i danych (ktorych nie ma): */
    ...
    /* Wysylanie pakietow co 1 sekunde: */
    for (;;) {
